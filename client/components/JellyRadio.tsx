...
export const JellyRadio: React.FC<JellyRadioProps> = ({
  ...
  const at = list.findIndex((it) => it.value === current);
  ...
  const apply = (sel: number, instant: boolean) => {
    ...
    const push = sel >= 0 ? ((widths.current[sel] ?? 0) * C.swell) / 2 + C.barge : 0;
    for (let i = 0; i < C.count; i++) {
      const mv = mvFor(i);
      const on = i === sel;
      const far = sel >= 0 ? Math.abs(i - sel) : 0;
      const dir = sel >= 0 ? Math.sign(i - sel) * (rtl ? -1 : 1) : 0;
      const x = dir * push;
      const s = on ? 1 + C.swell : sel >= 0 ? 1 - C.shrink : 1;
      if (instant || C.reduce) {
        mv.x.jump(x);
        mv.sx.jump(s);
        mv.sy.jump(s);
  ...
  const commit = (i: number, instant: boolean) => {
    if (disabled || !list[i] || list[i].disabled) return;
    applied.current = i;
    apply(i, instant);
  ...
  return (
    <div
      ...
      className={`group inline-flex items-center select-none [-webkit-touch-callout:none] gap-[var(--jr-gap)] px-[var(--jr-pad-x)] py-[var(--jr-pad-y)] data-[disabled]:pointer-events-none data-[disabled]:opacity-50 ${className}`}
      style={
        {
          '--jr-chip': chipColor,
